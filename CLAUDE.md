...
`Desarrollos/github/chilean-altoutputs`, a private GitHub repository since 2026-09-24. Until then it
...
The deployed dashboard is password-protected at the hub level. How is documented in `_ref/` only.

## Deploy

Built and published by the metaudits hub (`rij/metaudits`), which reads this repository at
`Desarrollos/github/chilean-altoutputs`:

```bash
cd ../../rij/metaudits && npm run build:chilean-altoutputs && npm run deploy:quick
```

Served at https://metaudits.rijdho.org/chilean-altoutputs/ (password). No independent deploy.
